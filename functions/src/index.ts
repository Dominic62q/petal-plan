...
type ReminderJob = {
  ownerUid: string;
  itemType: "event" | "event-task" | "personal-task";
  itemId: string;
  eventId?: string;
  title: string;
  date: string;
...
function reminderDate(item: ReminderItem): { date: string; time: string } | null {
...
function scheduledFor(item: ReminderItem): Date | null {
  const when = reminderDate(item);
  const offset = item.reminderOffsetMinutes;
  if (!when || !item.timeZone || offset === undefined || !Number.isInteger(offset) || offset < 0) return null;
...
async function syncReminderJob({
  jobId,
  item,
  ownerUid,
  itemType,
  ...
  jobId: string;
  item: ReminderItem | undefined;
  ownerUid: string | undefined;
  itemType: ReminderJob["itemType"];
  itemId: string;
  eventId?: string;
  url: string;
}) {
...
export const syncEventReminder = onDocumentWritten(
  { document: "events/{eventId}", database: DATABASE_ID, region: REGION },
  async (event) => {
    const item = event.data?.after.exists
      ? event.data.after.data() as ReminderItem & { ownerUid?: string }
      : undefined;
...
export const syncEventTaskReminder = onDocumentWritten(
  { document: "events/{eventId}/tasks/{taskId}", database: DATABASE_ID, region: REGION },
  async (event) => {
    const item = event.data?.after.exists ? event.data.after.data() as ReminderItem : undefined;
    const eventSnapshot = await db.collection("events").doc(event.params.eventId).get();
    const ownerUid = eventSnapshot.exists ? eventSnapshot.get("ownerUid") as string | undefined : undefined;
    await syncReminderJob({
...
export const syncPersonalTaskReminder = onDocumentWritten(
  { document: "users/{uid}/standalone_tasks/{taskId}", database: DATABASE_ID, region: REGION },
  async (event) => {
    const item = event.data?.after.exists ? event.data.after.data() as ReminderItem : undefined;
    await syncReminderJob({
...
async function sendToUser(ownerUid: string, payload: object) {
  configureWebPush();
  ...
  await Promise.all(subscriptions.docs.map(async (subscription) => {
    const data = subscription.data() as StoredPushSubscription;
    try {
      ...
      const statusCode = typeof error === "object" && error !== null && "statusCode" in error
        ...
        : 0;
      if (statusCode === 404 || statusCode === 410) await subscription.ref.delete();
      else logger.error("Unable to send push notification", { ownerUid, subscriptionId: subscription.id, error });
    }
  }));
  return delivered;
}
  ...
  async () => {
    ...
    await Promise.all(dueJobs.docs.map(async (candidate) => {
      const claimed = await db.runTransaction(async (transaction) => {
        ...
        const job = current.data() as ReminderJob | undefined;
        if (!current.exists || job?.status !== "pending" || job.scheduledFor.toMillis() > now.toMillis()) return null;
...
export const sendTestReminder = onCall(
  { region: REGION, secrets: [VAPID_SUBJECT, VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY] },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Sign in before testing notifications.");
